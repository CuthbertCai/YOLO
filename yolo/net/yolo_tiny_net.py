...
#        self.trainable_collection = []
    ...
    def inference(self, images):
        ...
        """
        net = self.conv2d('conv1', images, [3, 3, 3, 16])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv2', net, [3, 3, 16, 32])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv3', net, [3, 3, 32, 64])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv4', net, [3, 3, 64, 128])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv5', net, [3, 3, 128, 256])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv6', net, [3, 3, 256, 512])
        net = self.max_pool(net, [2, 2], 2)
        net = self.conv2d('conv7', net, [3, 3, 512, 1024])
        net = self.conv2d('conv8', net, [3, 3, 1024, 1024])
        net = self.conv2d('conv9', net, [3, 3, 1024, 1024])

        net = tf.transpose(net, [0, 3, 1, 2])

        #Fully connected layers
        net = self.local('local1', net, self.cell_size * self.cell_size * 1024, 256)
        net = self.local('local2', net, 256, 4096)
        net = self.local('local3', net, 4096,
        ...
        class_probs = tf.reshape(net[:, 0: n1], [-1, self.cell_size, self.cell_size, self.num_classes])
        scales = tf.reshape(net[:, n1: n2], [-1, self.cell_size, self.cell_size, self.boxes_per_cell])
        boxes = tf.reshape(net[:, n2:], [-1, self.cell_size, self.cell_size, self.boxes_per_cell * 4])

        net = tf.concat([class_probs, scales, boxes], 3)
